...
if __name__ == '__main__':
    ...
    view_path = create_views.create_view(url=url, db_name=db_name, view_name='sentiment_distribution', mapFunc=SENTIMENT_DISTRIBUTION_VIEW, overwrite=True)
    ...
    for each in view:
        ...
        if 'coordinates' in each.value.keys():
            ...
            if 'coordinates' in each.value['place']['bounding_box'].keys():
                ...
                temp_dict['sentiment'] = each.value['sentiment']
        temp_dict['text'] = each.value['text']
